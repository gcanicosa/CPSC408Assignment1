...
#display_students() shows all students a specified attribute
def display_students():
    ...
    while isStudRecordNotDisplayed:
        ...
        if response == "1":
            ...
            results = mycursor.execute("SELECT * FROM Students WHERE Major = ?", (major,))
        elif response == "2":
            gpa = input("Enter a GPA:")
            results = mycursor.execute("SELECT * FROM Students WHERE GPA = ?", (gpa,))
        elif response == "3":
            city = input("Enter a city:")
            results = mycursor.execute("SELECT * FROM Students WHERE City = ?", (city,))
        elif response == "4":
            city = input("Enter a state:")
            results = mycursor.execute("SELECT * FROM Students WHERE State = ?", (state,))
        elif response == "5":
            facAdvisor = input("Enter a faculty advisory:")
            results = mycursor.execute("SELECT * FROM Students WHERE FacultyAdvisor = ?", (facAdvisor,))
        else:
